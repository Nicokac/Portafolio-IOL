...
def test_logout_forces_login_page(monkeypatch):
    ...
    cache.session_state["IOL_USERNAME"] = "user"
    ...
    assert cache.session_state.get("force_login") is True
...
def test_service_build_iol_client_returns_error(monkeypatch):
    ...
    cache.session_state.clear()
    cache.session_state["IOL_USERNAME"] = "user"
    cache.session_state["IOL_PASSWORD"] = "pass"
...
def test_controller_build_iol_client_handles_error(monkeypatch):
    ...
    assert cache.session_state.get("force_login") is True
    assert cache.session_state.get("login_error") == "bad creds"
    assert cache.session_state.get("IOL_PASSWORD") == ""
...
def test_controller_build_iol_client_success_clears_password(monkeypatch):
    ...
    assert cache.session_state.get("authenticated") is True
    assert "IOL_PASSWORD" not in cache.session_state


def test_render_login_page_shows_error(monkeypatch):
    cache.session_state.clear()
    cache.session_state["login_error"] = "fail"
